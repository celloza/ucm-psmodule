...
.EXAMPLE
    New-UcmQueue -Uri http://10.10.10.1/api -Cookie $cookie -Extension "1001" -QueueName "SupportQueue" `
...
function New-UcmQueue {
    ...
    if ($responseContent.status -ne 0) {
        Write-Error "API call to addQueue failed. Status: $($responseContent.status)"
    } else {
