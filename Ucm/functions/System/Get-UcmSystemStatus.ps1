...
.DESCRIPTION
    This cmdlet corresponds to the API method "getSystemStatus" and performs operations related to SystemStatus.
...
.EXAMPLE
    # Example usage
    Get-UcmSystemStatus -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Get-UcmSystemStatus
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to getSystemStatus failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
