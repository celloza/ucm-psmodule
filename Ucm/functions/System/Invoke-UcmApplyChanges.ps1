...
.DESCRIPTION
    This cmdlet corresponds to the API method "applyChanges" and performs operations related to ApplyChanges.
...
.EXAMPLE
    # Example usage
    Invoke-UcmApplyChanges -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmApplyChanges
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to applyChanges failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
