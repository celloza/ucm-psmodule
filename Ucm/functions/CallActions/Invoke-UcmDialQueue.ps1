<#
.SYNOPSIS
    Invoke the DialQueue using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "dialQueue" and performs operations related to DialQueue.
...
.EXAMPLE
    # Example usage
    Invoke-UcmDialQueue -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmDialQueue
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to dialQueue failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
