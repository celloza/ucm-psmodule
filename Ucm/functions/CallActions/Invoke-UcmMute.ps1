<#
.SYNOPSIS
    Invoke the Mute using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "mute" and performs operations related to Mute.
...
.EXAMPLE
    # Example usage
    Invoke-UcmMute -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmMute
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to mute failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
