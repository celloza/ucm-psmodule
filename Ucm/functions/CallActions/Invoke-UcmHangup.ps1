<#
.SYNOPSIS
    Invoke the Hangup using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "Hangup" and performs operations related to Hangup.
...
.EXAMPLE
    # Example usage
    Invoke-UcmHangup -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmHangup
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to Hangup failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
