...
.DESCRIPTION
    This cmdlet corresponds to the API method "transferNumberOutbound" and performs operations related to TransferNumberOutbound.
...
.EXAMPLE
    # Example usage
    Invoke-UcmTransferNumberOutbound -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmTransferNumberOutbound
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to transferNumberOutbound failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
