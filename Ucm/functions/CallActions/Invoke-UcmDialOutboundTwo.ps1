<#
.SYNOPSIS
    Invoke the DialOutboundTwo using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "dialOutboundTwo" and performs operations related to DialOutboundTwo.
...
.EXAMPLE
    # Example usage
    Invoke-UcmDialOutboundTwo -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmDialOutboundTwo
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to dialOutboundTwo failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
