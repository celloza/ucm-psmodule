<#
.SYNOPSIS
    Invoke the DialOutbound using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "dialOutbound" and performs operations related to DialOutbound.
...
.EXAMPLE
    # Example usage
    Invoke-UcmDialOutbound -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmDialOutbound
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to dialOutbound failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
