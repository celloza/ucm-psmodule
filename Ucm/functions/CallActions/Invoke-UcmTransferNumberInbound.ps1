<#
.SYNOPSIS
    Invoke the TransferNumberInbound using the UCM API.

.DESCRIPTION
    This cmdlet corresponds to the API method "transferNumberInbound" and performs operations related to TransferNumberInbound.
...
.EXAMPLE
    # Example usage
    Invoke-UcmTransferNumberInbound -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmTransferNumberInbound
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to transferNumberInbound failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
