...
.DESCRIPTION
    This cmdlet corresponds to the API method "hold" and performs operations related to Hold.
...
.EXAMPLE
    # Example usage
    Invoke-UcmHold -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmHold
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to hold failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
