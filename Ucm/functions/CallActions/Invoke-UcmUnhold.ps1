...
.DESCRIPTION
    This cmdlet corresponds to the API method "unhold" and performs operations related to Unhold.
...
.EXAMPLE
    # Example usage
    Invoke-UcmUnhold -Uri http://10.10.10.1:80/api -Cookie "session_cookie"
#>
function Invoke-UcmUnhold
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
    ...
    {
        Write-Error "API call to unhold failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
