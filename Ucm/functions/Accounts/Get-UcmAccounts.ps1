...
.DESCRIPTION
    This cmdlet corresponds to the API method "listAccount" and performs operations related to Accounts.
...
.EXAMPLE
    # Example usage
    Get-UcmAccounts -Uri http://10.10.10.1:80/api -Cookie "session_cookie" -Top 15 -Page 2 -SortOrder "desc"
#>
function Get-UcmAccounts
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory)]
        ...
        [Int32]$Top = 30,
        [Int32]$Page = 1,
        [string]$Fields = "extension,account_type,fullname,out_of_service,status,addr,urgemsg,newms,oldmsg",
    ...
    {
        Write-Error "API call to listAccount failed. Status code was $((ConvertFrom-Json $apiResponse.Content).status)."
    }
    else
    {
